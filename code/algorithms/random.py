...
class RandomFold:
    ...
    def __init__(self, protein: Protein, dimensions: int,
                 avoid_overlap: Optional[bool] = True) -> None:
        """
    ...
    def backtracking(self, max_backtracking: int = 5000) -> None:
        ...
        while acid_index < len(self._protein._sequence):
            if acid.predecessor:
                ...
                if not directions:
                    # backtracking
                    avoid_direction_path = tuple(map(sub, protein_path[acid_index - 2], protein_path[acid_index - 3]))
                    avoid_direction_failed_path = tuple(map(sub, protein_path[acid_index - 1], protein_path[acid_index - 2]))
                    directions = self._get_directions()
                    ...
                    print(f"{acid}[{acid_index}]: {acid.position}")
