        ...
        for row in rows:
            if row[0].startswith('Average:'):
                try:
                    ...
                    print(f"Warning: Could not parse average value '{row[1]}' as float.")
                    avg.append(None)
            else:
                try:
                    seq_values.append(float(row[-1]))
                    if len(seq_values) == 1:  # First entry of a new sequence
                        names.append(row[1])  # Assuming the protein chain name is in the second column
                except ValueError:
                    print(f"Warning: Could not parse value '{row[-1]}' as float.")
...
def plot_combined_histogram(data, averages, names, algorithm_names, output_folder, file_name, dimension):
    ...
    lighter_colors = ['#add8e6', '#ffd699', '#90ee90', '#c5b0d5']  # Lighter shades for 4 algorithms
    ...
    for i in range(len(data[0])):
        ...
        min_score, max_score = min(map(min, iteration_scores)), max(map(max, iteration_scores))
        ...
        for j, (seq_data, avg) in enumerate(zip(data, averages)):
            ...
            ax.bar(score_range - (width * num_algorithms / 2) + j * width, score_counts, width, color=colors[j], label=f"{algorithm_names[j]}")

            # Add a line for the average with a darker color
            ax.axvline(x=avg[i], color=colors[j], linestyle='dashed', linewidth=1.5)

            # Add a line for the minimum with a lighter color
            min_val = min(seq_data[i])
            ax.axvline(x=min_val, color=lighter_colors[j], linestyle='dotted', linewidth=1.5)

       # Create custom handles for the legend
        handles, labels = ax.get_legend_handles_labels()
        avg_line = plt.Line2D([0], [0], color='black', linestyle='dashed', linewidth=1.5, label='Avg')
        min_line = plt.Line2D([0], [0], color='gray', linestyle='dotted', linewidth=1.5, label='Min')
        ...
        ax.legend(handles=handles, fontsize='small', loc='upper left', bbox_to_anchor=(0, 1))
        plt.grid(True)
...
def main():
    parser = argparse.ArgumentParser(description="Generate plots for protein chain analysis.")
    parser.add_argument("number", type=int, choices=[2, 3], help="Number representing the dimension (2D or 3D).")
    parser.add_argument("letter", type=str, nargs='?', choices=['c', 'C',''], default='', help="Letter representing the type (e.g., C), optional.")
    ...
    plot_combined_histogram(data, averages, names, algorithm_names, output_path, file_name, f"{dimension}D")
